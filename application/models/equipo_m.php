...
class Equipo_m extends MY_Model 
{
	...
	{
		$query = $this->db->query("SELECT *, (SELECT COUNT(*) from team_match t4 where t4.id_team_1 = t1.id OR t4.id_team_2 = t1.id) as EJ, 	(SELECT COUNT(*) from team_match t4 where (t4.id_team_1 = t1.id AND t4.team_1_score > t4.team_2_score) OR (t4.id_team_2 = t1.id AND t4.team_2_score > t4.team_1_score)) as EG,	(SELECT COALESCE(SUM(t2.team_1_score), 0) from team_match t2 where t2.id_team_1 = t1.id) + (SELECT COALESCE(SUM(t3.team_2_score), 0) from team_match t3 where t3.id_team_2 = t1.id) as JG,	(SELECT SUM(score) from score t5 where t5.id_team = t1.id) as GF,	(SELECT SUM(score) from score t6 inner join game t7 on t6.id_game = t7.id inner join team_match t8 on t8.id = t7.id_match where (t8.id_team_1 = t1.id OR t8.id_team_2 = t1.id) AND (t6.id_team != t1.id)) as GC	from team t1 ", FALSE);
		return $query->result();
	}	
